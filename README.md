...
- AC-coupled **and** DC-coupled PV capture (SmartSolar / MPPT chargers, via the system service)
...
Minimal (defaults for everything except the two hosts):
```bash
docker run -d \
...
Full form with all environment variables:
```bash
docker run -d \
    -e INFLUXDB=<hostname/IP of InfluxDB server - default=localhost> \
    -e INFLUXPORT=<port InfluxDB is running on - default=8086> \
    -e VENUS=<hostname/IP of Venus Device - default=192.168.10.112> \
    -e VENUSPORT=<ModbusTCP port on Venus device - default=502> \
    -e UNITID=<Modbus ID of Venus Device - default=100> \
    fidozbox/victronvenus-influxdb
```

Replace the user variables defined by `<>` with the correct values. Environment
variables can be excluded if the defaults are suitable.
...
| VENUS | Venus device host | 192.168.10.112 |
...
6. Capture DC-coupled PV (SmartSolar / MPPT) from system registers 850/851

## Data structure notes

Registers are read as three separate blocks against unit-id 100
(`com.victronenergy.system`). Offsets below are relative to each block's start.

    # --- SYSTEM_INFO block : start 800, count 27 ---
    # reg_block[0]  = Serial
    # reg_block[1]  = Serial
    # reg_block[2]  = Serial
    # reg_block[3]  = Serial
    ...
    # --- BATTERY_INFO block : start 840, count 7 ---
    ...
    # --- DC_PV_INFO block : start 850, count 2 --- (SmartSolar / MPPT)
    # reg_block[0] = PV - DC-coupled power (System)    -> reg 850, uint16, W
    #                (/Dc/Pv/Power  - sum of output power of all solar chargers)
    # reg_block[1] = PV - DC-coupled current (System)  -> reg 851, int16, x10 = 0.1 A DC
    #                (/Dc/Pv/Current - signed, wrap in np.int16 before scaling)
    #
    # Note: DC-coupled = side batterie/bus DC. The SmartSolar output does NOT
    # appear in the AC-coupled PV registers above; it is aggregated here at the
    # system level. Per-charger PV-side V/I and kWh yield live in the
    # com.victronenergy.solarcharger service (different unit-id), not here.
